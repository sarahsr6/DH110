...
| 6. Recognition Rather than Recall | Discord does do a good job of displaying all needed information at all times. So much so that it can get chaotic at times. One thing I have trouble with is remembering my username to tell friends. On the computer it is visual at all times which is very convinient. Howeer, on the phone app, you have to search for it which can be tricky. | Maybe make username visable on phone app at all times as well. Thay comes up alot when connecting with new friends. | 1 |
| 7. Flexibility and Efficiency of Use | Discord has plenty shortcuts for more experienced users, which is very nice. They have a variety of keybord commands that indicate shortcuts for almost everything. But new users can stick to clicking the buttons directly on the screen. | They have so many buttons to do one command, and it can be too much sometimes. Maybe have max 2 ways to do one action. | 1 |
| 8. Aesthetic and Minimalistic Design | Discord has a bit too much going on tho the point where it is overwhelming to new users. Especially if you are in multiple servers, things get very cluttered and chaotic at times. It shows every single server, alongside all the channels in the current server and all the messages of a chat while chatting to friends. So essentially, you can be in a chat channel and call at the same time and not know which chat to look at. | Discord doesnt need to show all this because it makes the exit buttons less noticable, which can be frustrating. They should also have a single chatchannel dedicated to each all channel so people know where to look. | 3 |
| 9. Help Users Recognise, Diagnose, and Recover from Errors | Discord is a mix of good and bad when diagnosing errors. It is able to detect easy errors like wifi, and communicates it as a pop up saying "oops, something went wrong". It even goes as far to help you by suggesting fixes which is very convinient for users. They also have a button for help at the top that takes you to a link where you can ask any questions. But there are still issues sometimes with sharing screens where the content won't show up. It doent recognize these errors which makes it harder to fix. | Maybe they need a pop up or manual when it comes to sharing screens, because that is a big factor of discord.  | 1 |
| 10. Help and Documentation | Discord has a very nice manual for help. One, the have a nice button that you can ask questins. THen they also have multible other manuals showing you any shortcut keys and more. | Discord has very good help options already, I would just suggest a better training module for new users. | 1 |
